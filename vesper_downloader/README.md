...
- In `pipeline_url` mode the app resolves the root run, traverses downstream runs, and downloads artifacts for each discovered run. For Pipeline roots it first tries classic Jenkins run JSON and then falls back to `wfapi` stage logs when needed.
...
- If `input.pipeline_url` belongs to a skipped job, or if the skip filtering leaves nothing to process, the downloader exits with an error.
