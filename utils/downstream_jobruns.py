...
from .jenkins_helpers import canonicalize_run_url, compose_job_path, create_url_opener, fetch_url_json
...
def _create_build_fetcher(
    ...
    def fetch_build_payload(normalized_build_run_url) -> dict[str, Any]:
        ...
        api_endpoint_url = f"{canonicalize_run_url(normalized_build_run_url)}api/json?tree={api_tree}"
...
def _resolve_child_run_url(item_payload, parent_base_url) -> str | None:
    ...
    if isinstance(direct_run_url, str) and direct_run_url.strip():
        if "://" in direct_run_url:
            resolved_child_url = canonicalize_run_url(direct_run_url)
        ...
        resolved_child_url = canonicalize_run_url(
    ...
    if isinstance(job_base_url, str) and job_base_url.strip():
        resolved_child_url = canonicalize_run_url(f"{canonicalize_run_url(job_base_url)}{resolved_build_number}/")
...
def discover_downstream_jobrun_urls(
    ...
    canonical_root_run_url = canonicalize_run_url(pipeline_execution_url)
